...
- **Rich mouse vocabulary** — `click` supports OpenAI's `left`, `right`, `wheel` (middle), `back`, and `forward` buttons plus a bounded `click_count`; `scroll` uses `scrollX` / `scrollY`; `drag` uses `path`.
