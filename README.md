...
- **Rich mouse vocabulary** — `click` supports `mouse_button` and a bounded `click_count`; `scroll` uses `scrollX` / `scrollY`; `drag` uses `path`.
- **OpenAI action spelling compatibility** — `double_click`, `type`, and `keypress { keys: [...] }` are accepted alongside the Windows canonical `click`, `type_text`, and `press_key` actions.
