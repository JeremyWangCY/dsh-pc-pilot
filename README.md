...
| `browser_tabs` / `browser_state` / `browser_observe` / `browser_history` / `browser_back` / `browser_forward` / `browser_wait` | Manage exact tabs, inspect semantic state with compact refs, navigate history, and wait on page readiness/URL/text without synthetic sleeps |
