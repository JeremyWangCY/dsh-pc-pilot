...
export function defineComputerTool(defineTool, ctx) {
  ...
  const parameters = {
      action: {
        ...
        enum: [
          ...
          'browser_open', 'browser_navigate', 'browser_activate', 'browser_close', 'browser_shutdown', 'browser_state', 'browser_read', 'browser_request', 'browser_click', 'browser_click_text', 'browser_type', 'browser_replace', 'browser_key', 'browser_scroll', 'browser_scroll_to_text', 'browser_reload',
        ],
        ...
        try {
          ...
          const browserObservation = browserResult.screenshot
            ? { ok: true, action: 'screenshot', outcome: 'completed', screenshot: browserResult.screenshot }
             : (['browser_state', 'browser_read', 'browser_request', 'browser_open', 'browser_navigate', 'browser_activate', 'browser_close', 'browser_shutdown'].includes(action) ? null : { ok: false, action: 'screenshot', outcome: 'unknown', error_code: 'observation_failed', message: 'browser mutation completed without a post-action screenshot' })
  ...
  async function executeAction(args, exec) {
    ...
    const inherited = {}
    for (const key of ['app', 'hwnd', 'window_index', 'snapshot_id', 'browser_endpoint', 'tab_id', 'dispatch', 'overlay', 'display', 'confirmation', 'include_text', 'wait_for']) {
