...
function acquireDaemon() {
  ...
  return ensureHelper().then(() => new Promise((resolve, reject) => {
    let child
    try {
      child = spawn(PS_EXE, [
        '-NoProfile', '-ExecutionPolicy', 'Bypass',
    ...
    // stay covered by the ref'd 60s per-request timer.
    child.unref()
    child.stdout.unref()
    child.stderr.unref()
    child.stdin.unref()
...
function daemonRequest(action, args, signal) {
  ...
  return acquireDaemon().then((child) => new Promise((resolve, reject) => {
    ...
    entry.resolve = (msg) => finish(resolve, msg)
    entry.reject = (err) => finish(reject, err)
    daemon.pending.set(id, entry)
    entry.timer = setTimeout(() => {
      finish(reject, new Error('daemon request timed out after 60000ms'))
