...
export function defineComputerTool(defineTool, ctx) {
  ...
  async function observeForPostcondition(expect, requestArgs, actionResult, signal) {
    ...
    const target = { app: requestArgs.app, hwnd: requestArgs.hwnd, window_index: requestArgs.window_index, dispatch: 'background' }
  ...
  async function maybeRecoverForeground(result, requestArgs, action, invokeNative, signal) {
    ...
    if (retryArgs.element !== undefined) {
      const target = { app: retryArgs.app, hwnd: retryArgs.hwnd, window_index: retryArgs.window_index, screenshot: false, include_text: true, dispatch: 'background' }
      let state
  ...
  async function executeAction(args, exec) {
    ...
    const inherited = {}
    for (const key of ['app', 'hwnd', 'window_index', 'snapshot_id', 'browser_endpoint', 'tab_id', 'dispatch', 'overlay', 'display', 'confirmation', 'include_text', 'wait_for', 'recovery']) {
