...
const toolDescription = [
  ...
  PC_PILOT_SKILL_HINT,
  '',
  'Use the identifiers the tool returns: keep the same window, browser_endpoint, and tab_id through a task. get_window_state is screenshot-first; request include_text only when indexed UIA elements, document text, or accessibility status are needed. Every element_index must use the snapshot_id from the same state.',
  '',
  ...
  '',
  'Browser element tokens are bound to their current page. Keep the current tab and recover in place rather than restarting the browser. For a mutating action whose outcome is unknown, inspect the same target and do not repeat the mutation automatically.',
  '',
...
export function defineComputerTool(defineTool, ctx, options = {}) {
  const browserProvider = options.browserProvider || ctx?.browserProvider || defaultBrowserProvider
  ...
  const parameters = {
      action: {
        ...
        enum: [
          ...
          'browser_open', 'browser_navigate', 'browser_activate', 'browser_close', 'browser_shutdown', 'browser_tabs', 'browser_events', 'browser_downloads', 'browser_history', 'browser_back', 'browser_forward', 'browser_wait', 'browser_state', 'browser_observe', 'browser_read', 'browser_request', 'browser_click', 'browser_click_text', 'browser_type', 'browser_replace', 'browser_key', 'browser_scroll', 'browser_scroll_to_text', 'browser_reload',
        ],
      ...
      browser_element: { type: 'string', description: 'Element token or short @eN ref from the latest browser_state/browser_observe; use this for browser_click/browser_type/browser_replace/browser_key.' },
  ...
  async function executeSingleAction(args, exec, options = {}) {
      ...
      if (action.startsWith('browser_')) {
        ...
        try {
          ...
          const value = { ...browserResult, ok: browserResult.ok !== false && !observationFailed, action, outcome: browserResult.outcome || (observationFailed ? 'unknown' : ['browser_state', 'browser_observe'].includes(action) ? 'completed' : 'dispatched'),
