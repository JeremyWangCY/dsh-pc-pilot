...
const HELPER_TARGET = path.join(TEMP_DIR, 'dsh-cua-helper.ps1')
...
const PIP_SOURCE = path.join(__dirname, 'pip-overlay.ps1')
const PIP_TARGET = path.join(TEMP_DIR, 'pip-overlay.ps1')
const SETUP_SOURCE = path.join(__dirname, 'setup-virtual-display.ps1')
const SETUP_TARGET = path.join(TEMP_DIR, 'setup-virtual-display.ps1')
...
function ensureHelper() {
  helperReady ??= (async () => {
    ...
    try { fs.copyFileSync(PIP_SOURCE, PIP_TARGET) } catch { /* pip optional */ }
    try { fs.copyFileSync(SETUP_SOURCE, SETUP_TARGET) } catch { /* setup optional */ }
...
3. Act on the state: click_element { app, element }, click { app, x, y } (button 'right' for right-click, click_count 2/3 for double/triple), mouse_move { app, x, y }, mouse_down { app, x, y, button }, mouse_up { app, x, y, button }, set_value { app, element, value }, type { app, text } (or type { app, element, text } to target a specific element), perform_action { app, element, perform } (invoke/toggle/select/expand/collapse/focus/scroll), select_text { app, element, start, length }, key { app, key, modifiers }, hold_key { app, key, duration_ms, modifiers }, scroll { app, x, y, amount, direction (down/up/left/right) }, drag { app, from_x, from_y, to_x, to_y }, or open_app { name }.
4. Window management & utilities: get_window { app, hwnd } (fresh metadata: rect, minimized, title, pid, process_name), activate_window { app, hwnd } (bring to foreground), close_window { app, hwnd } (graceful WM_CLOSE), list_windows { app }, screenshot { display, x, y, width, height } (x/y/width/height are absolute screen coordinates, intersected and clamped with the display bounds), zoom { path, x, y, width, height }, switch_display { display }, cursor_position, wait { duration_s }, read_clipboard, write_clipboard { text }, list_displays.
  ...
  const parameters = {
      action: {
        ...
        enum: [
          ...
          'activate_window', 'close_window', 'get_window',
          'toggle_pip', 'isolate_window', 'release_window',
      ...
      element: { type: 'number', description: 'Element index from the latest get_app_state (click_element / set_value / perform_action / select_text / type).' },
      ...
      key: { type: 'string', description: 'Key name or chord for key action: Return, Enter, Escape, ctrl+c, Control_L+a, alt+f4, ctrl+shift+p, Tab, Backspace, Delete, Home, End, PageUp, PageDown, Arrow keys, Space, PrintScreen, CapsLock, F1-F24, a-z, 0-9, punctuation.' },
      ...
      perform: { type: 'string', description: 'Action name for perform_action: invoke (aliases press/click), toggle (alias switch), select, add_to_selection, remove_from_selection, expand, collapse, focus (alias set_focus), scroll_up, scroll_down, scroll_left, scroll_right.' },
      ...
      path: { type: 'string', description: 'Source screenshot path for zoom (default: newest shot).' },
      ...
      show: { type: 'boolean', description: 'Show or hide the Apple-style PiP floating preview window (toggle_pip).' },
      isolate: { type: 'boolean', description: 'Place target window on the isolated virtual display canvas (open_app, isolate_window).' },
      restore: { type: 'boolean', description: 'Restore isolated window from virtual canvas back to primary screen (isolate_window).' },
      all: { type: 'boolean', description: 'Release all canvas windows back to primary screen (release_window).' },
      setup: { type: 'boolean', description: 'With setup_virtual_display: auto = install driver + activate canvas if missing (default). status/install/activate run a single stage.' },
  }
...
export function apply(ctx) {
  ...
  // Full product: the virtual display canvas is part of the plugin, not an optional
  // extra. After tool registration, run the idempotent setup once per host boot in
  // the background — when the driver is missing it installs silently and the UAC
  // prompt (one click) is the user's consent gate; on decline the plugin still
  // works via off-desktop parking. Never blocks startup, never repeats if present.
  const ensureVirtualDisplay = () => {
    let out = ''
    const child = spawn(PS_EXE, [
      '-NoProfile', '-ExecutionPolicy', 'Bypass',
      '-File', SETUP_TARGET, '-Action', 'auto',
    ], { cwd: SYSTEM_ROOT, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] })
    child.unref()
    child.stdout.setEncoding('utf8')
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        diag('virtual display ready: ' + (m ? m[0] : 'ready'))
      } else {
        diag('virtual display setup incomplete (exit=' + code + '): ' + String(out).slice(-400))
      }
    })
    child.on('error', (e) => diag('virtual display setup spawn failed: ' + (e && e.message)))
  }
  const registerWith = (defineTool) => {
    ...
    ensureHelper()
      .then(() => { const t = setTimeout(ensureVirtualDisplay, 4000); if (t.unref) t.unref() })
      .catch(() => { /* helper distribution failed; nothing to bootstrap */ })
  }
