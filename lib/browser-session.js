    ...
    send(method, params = {}, sessionId, options = {}) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
          ...
          pending.delete(id)
          cleanup()
          reject(new Error(`CDP command timeout (${timeoutMs}ms)`))
...
export async function browserAction(action, args = {}, signal) {
  if (!['browser_open', 'browser_navigate', 'browser_activate', 'browser_close', 'browser_shutdown', 'browser_tabs', 'browser_events', 'browser_downloads', 'browser_history', 'browser_back', 'browser_forward', 'browser_wait', 'browser_state', 'browser_read', 'browser_request', 'browser_click', 'browser_click_text', 'browser_type', 'browser_replace', 'browser_key', 'browser_scroll', 'browser_scroll_to_text', 'browser_reload'].includes(action)) throw new Error('Unsupported browser action')
  ...
  try {
    ...
    cdp.prunePageSessions(pages.map(page => page.targetId))
    ...
    if (action === 'browser_state') {
      ...
      const stateScreenshot = args.with_screenshot ? await capturePageScreenshot(send, commandTimeoutMs) : null
      return {
        tab_id: args.tab_id,
        browser_session_id: cdp.sessionId(),
        url: pageUrl,
