...
- Reuse the returned \`window\`, \`browser_endpoint\`, and \`tab_id\`. Recover in place before opening or launching a replacement.
...
- An \`element_index\` is valid only with the \`snapshot_id\` from the same state. Browser element tokens are valid only for their current document.
- Reuse a returned \`post_action_observation\` when it already contains the evidence needed for the next decision. Do not call \`get_window_state\` or \`browser_state\` merely because an action occurred.
...
- Use \`expect\` when there is a concrete success condition. Treat an \`unknown\` mutation outcome as uncertain and inspect the same target; do not repeat the mutation automatically.
...
## Browser tasks
- Keep one exact \`browser_endpoint\` and \`tab_id\`. Use \`browser_state\` to select or refresh a tab when needed, not as a mandatory extra step after every action.
