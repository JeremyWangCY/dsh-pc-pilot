...
- After launching or when a modern app is loading, use \`wait { window, duration_s, include_text: true }\`. Its post-wait observation reports \`accessibility_status\`: \`available\` can supply element indexes, \`partial\` needs another careful observation, and \`unavailable\` means no UIA target exists. Do not invent an element index; wait/re-observe, or use a screenshot-bound foreground coordinate path only when the user has authorized foreground control.
