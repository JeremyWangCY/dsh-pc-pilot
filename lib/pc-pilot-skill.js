...
PC-Pilot uses the ChatGPT Windows Computer Use desktop vocabulary directly: \`list_apps\`, \`list_windows\`, \`get_window\`, \`launch_app\`, \`get_window_state\`, \`click\`, \`press_key\`, \`type_text\`, \`scroll\`, \`drag\`, \`set_value\`, \`perform_secondary_action\`, and \`activate_window\`. It also accepts OpenAI Responses spellings \`double_click\`, \`type\`, and \`keypress { keys: [...] }\`, normalizing them to the same native behavior. Pass a returned \`window: { id, app }\` object unchanged; \`id\` is the target HWND. Use \`element_index\`, \`mouse_button\`, \`scrollX\`, \`scrollY\`, and \`include_screenshot\`. For \`perform_secondary_action\`, provide its label as \`secondary_action\`, because this single-tool envelope already uses \`action\` for the method name. Use \`browser_shutdown { browser_endpoint }\` only for cleanup after a browser task: it rejects endpoints not launched by this same PC-Pilot tool instance.

## Pick and preserve the target

- For a desktop app, start with \`list_apps\`, choose one returned window, then call \`get_window_state { window, include_screenshot: true }\`. Keep that app/window for the task. Do not open a replacement window because one control, screenshot, or wait failed. \`launch_app\` is minimized by default; if the user explicitly asked for a foreground app or a modern app needs foreground UIA, use \`launch_app { activate: true }\` and then observe its returned \`window\`.
...
- A desktop \`get_window_state\` is screenshot-first. Request \`include_text: true\` when you need indexed elements; it then yields a \`snapshot_id\` whose element actions must carry that same \`snapshot_id\`. Any mutation, navigation, scrolling, or meaningful delay invalidates it: obtain a new state before the next element action.
- After launching or when a modern app is loading, use \`wait { window, duration_s, wait_for: "accessibility_present", include_text: true }\` to wait for any usable UIA descendant, or \`wait_for: "accessibility_available"\` for a complete tree. A condition timeout is safe to retry and returns the last \`accessibility_status\`. Its post-wait observation reports \`available\`, \`partial\`, or \`unavailable\`: do not invent an element index; wait/re-observe, or use a screenshot-bound foreground coordinate path only when the user has authorized foreground control.
...
- On a stale token, missing control, loading page, timeout, background-unavailable, or foreground_activation_unconfirmed result, refresh the current target and take the narrowest evidence-backed recovery. \`foreground_activation_unconfirmed\` means no real input was sent; if it includes \`launch_succeeded: true\`, observe the returned window and never retry the launch. Do not re-search, create another browser, reload blindly, or close the working window.
